...
public class Grid {
    ...
    public void Spill(int centerRow, int centerCol, int initStrength){
        Q = new ArrayDeque<Integer>();
        SpillInCell(centerRow, centerCol, initStrength);
        while (!Q.isEmpty()){
            int x = Q.remove();
            int y = Q.remove();
            int strength = getCell(x, y).getValue();
    ...
    void SpillInCell(int x, int y, int strength){
        ...
        if (x<0 || x>=getRows() || y<0 || y>=getColumns()) return;
        int cell = getCell(x, y).getValue();
        if (isObstacle(x, y)) return;
        if (cell >= strength) return;
        setCell(x, y, strength);
        Q.add(x);
        Q.add(y);
    }
